...
void Matrix4::Rotate(float iAngle, const Vector3& iVector) {
    ...
    Vector4 x = Vector4(m[0]) * rotation.m[0][0] + Vector4(m[1]) * rotation.m[0][1] +
                Vector4(m[2]) * rotation.m[0][2];
    Vector4 y = Vector4(m[0]) * rotation.m[1][0] + Vector4(m[1]) * rotation.m[1][1] +
                Vector4(m[2]) * rotation.m[1][2];
    Vector4 z = Vector4(m[0]) * rotation.m[2][0] + Vector4(m[1]) * rotation.m[2][1] +
                Vector4(m[2]) * rotation.m[2][2];
    m[0][0] = x.x();
    m[0][1] = x.y();
    m[0][2] = x.z();
    m[0][3] = x.w();
...
Vector4 Matrix4::operator*(const Vector4& iVector) const {
    return {iVector.x() * m[0][0] + iVector.y() * m[0][1] + iVector.z() * m[0][2] +
                iVector.w() * m[0][3],
...
// Statics
Matrix4 Matrix4::MakeProjectionOrtho(float iLeft,
                                     float iRight,
                                     float iBottom,
                                     float iTop,
                                     float iNear,
