...
public:
    ...
    Quaternion(const Quaternion& iRhs)
        : m_W(iRhs.m_W), m_X(iRhs.m_X), m_Y(iRhs.m_Y), m_Z(iRhs.m_Z) {} // Copy ctor

    Quaternion(float iW, float iX, float iY, float iZ)
        : m_W(iW), m_X(iX), m_Y(iY), m_Z(iZ) {} // Explicit construction
    ...
    bool operator==(const Quaternion& iRhs)
        const { // TODO(pierre) We should probably use the utils FloatEqu here
        return m_X == iRhs.m_X && m_Y == iRhs.m_Y && m_Z == iRhs.m_Z && m_W == iRhs.m_W;
    }

    bool operator!=(const Quaternion& iRhs) { return !(*this == iRhs); }
